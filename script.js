...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const travelData = [
      {
        ...
        temperature: "12",
        description: "Đất nước đa dạng văn hóa, nhiều di tích lịch sử và danh lam thắng cảnh."
      },
      {
        country: "Nhật Bản",
        weather: "Ôn đới",
        temperature: "25",
        description: "Nổi tiếng với hoa anh đào, núi Phú Sĩ và văn hóa độc đáo."
      },
      {
        country: "Singapore",
        weather: "Nhiệt đới gió mùa",
        temperature: "25",
        description: "Quốc đảo hiện đại, sôi động với nhiều điểm tham quan hấp dẫn."
      },
    ];
    ...
    function handleSearchClick() {
      ...
      for (const travelItem of travelData) {
        if (country === travelItem.country.toLowerCase() && temperature === travelItem.temperature) {
          alert(`Bạn đã tìm thấy kết quả:
          - Quốc gia: ${travelItem.country} (${travelItem.weather})
          - Nhiệt độ: ${travelItem.temperature}
          - Mô tả: ${travelItem.description}`);
          foundResult = true;
          break; 
        }
      }
      ...
      if (!foundResult) {
        alert('Không tìm thấy địa điểm du lịch phù hợp với tiêu chí của bạn!');
      }
    }
